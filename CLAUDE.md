...
## Testing
Quick reference: 5 test layers — regression (static, 148 checks: 145 PASS / 3 advisory WARNING / 0 FAIL), install (static, 104 checks), unit (pytest, 458 tests), integration (e2e pipeline, 12 steps), behavior (API, ~15m). Structural PASS ≠ semantic PASS — run all static + integration tests for significant changes. Details below.
