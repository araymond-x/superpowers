...
- **Exit ladder**: **0** = spawned (`:484`) *or* `--dry-run` completed (`:439`) — and `launch=picker-manual` also exits 0 while a human must still finish the interactive picker in the new workspace before the pickup runs. **3** = manual fallback: five causes across six sites — cmux unreachable (`:125`), hop limit reached (`:137`), session quota below threshold (`:195`), either reservation write failed (`:464` `.handoff-hops`, `:469` spawn log), spawn failed after reservation (`:491`). **1** = refused precondition (bad args, not a git repo, missing `.active-feature`, dirty tree, bundle validation).
