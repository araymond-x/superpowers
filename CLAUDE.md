...
**Re-check (2026-07-21, STILL declined):** Upstream advanced to v6.1.1 (`d884ae0`, 2026-07-02; 188 commits ahead of our v5.1.0 merge base). The v6.0.3→v6.1.1 delta (19 commits) is entirely harness plumbing + token trims — Codex SessionStart re-registration fix + portal packaging script (v6.1.1), bootstrap compression, per-harness tool-mapping ref deletions, Codex marketplace install, and **Gemini CLI support removal** (Google EOL 2026-06-18) (v6.1.0). The only `skills/` changes are deletions/trims from the vendor-neutral rewrite this assessment already declined; the two 2-line `executing-plans`/`writing-skills` SKILL.md edits are just Gemini-ref/dead-link cleanups, not content fixes. **Nothing net-new for this Claude-Code-only symlink fork; no new hand-ports.** Adjacent idea only (not merged): v6.1.0 compressed the `using-superpowers` bootstrap for per-session token cost — our bootstrap has diverged too far to merge it, but the motivation applies more strongly here if a local trimming pass is ever wanted.
...
- **Where the RUNTIME contract lives — and why it is not here.** This file is a **project** CLAUDE.md: a session working in a *downstream* repo loads its own project file plus the globals, **never this one**. Only `skills/**` travels, via the `~/.claude/skills/superpowers` symlink (which points at `skills/`, not the repo root). So every fact a controller needs *while the gate is firing in another repo* lives in `references/context-handoff-protocol.md` — required picker/cmux env vars, the run-from-the-worktree rule, the preconditions, the silent `picker-manual` degradation, hop-budget scoping, and the log path. The hook's HARD-block message cites that file by path, so it is delivered at the moment of need. **Keep runtime facts there and maintainer facts here** (notify asymmetry, the bash-3.2/`set -u` trap, cross-repo ownership, line-anchored internals) — duplicating runtime facts into this file puts them where the audience that needs them cannot see them.
...
- **Exit ladder**: **0** = spawned (`:484`) *or* `--dry-run` completed (`:439`) — and `launch=picker-manual` also exits 0 while a human must still finish the interactive picker in the new workspace before the pickup runs. **3** = manual fallback: five causes across six sites — cmux unreachable (the `cmux ping` ≠ `PONG` probe), hop limit reached, session quota below threshold, either reservation write failed (`.handoff-hops` first, then the spawn log — two sites, one cause), spawn failed after reservation. `grep -nE '^[[:space:]]*exit 3'` enumerates exactly those six (anchor it — bare `grep 'exit 3'` also returns three comment lines, one of which describes the *decoder's* exit codes, not the ladder's). Left uncited *here* on purpose, because in this homogeneous list a bare line number that drifts lands on a *different* exit-3 branch and reads as plausible-but-wrong — whereas the next bullet's `:125`/`:464`/`:469` stay cited, because there each number is paired with an assertion about that branch's notify behavior, so a drift contradicts its own sentence and is caught. **1** = refused precondition (bad args, not a git repo, missing `.active-feature`, dirty tree, bundle validation).
...
- **cmux UI vocabulary — the vendored skill's wording reads BACKWARDS against the UI.** `cmux/SKILL.md` "Core Concepts" defines a **Workspace** as a *"tab-like group within a window"* and a **Surface** as *"a tab within a pane"*. In the actual UI it is the other way round from what that suggests: **workspaces are the LEFT-SIDEBAR list; surfaces are the TABS along the top.** So the one element described with the word "tab" is the one *not* drawn as tabs. Verified empirically (2026-07-27) against a live window: `cmux list-workspaces` returned exactly the sidebar entries in sidebar order, and `cmux list-pane-surfaces --workspace <ws>` returned exactly the top tabs with the same one marked `[selected]`. **Load-bearing for this feature:** `spawn-handoff-session.sh` calls `cmux new-workspace`, so a spawned successor appears as a **new left-sidebar entry, NOT a new tab** — and the success notify/stdout both name it as `workspace:<N>`. Anyone who reads "tab-like group" and searches the tab bar will conclude the spawn failed when it succeeded. Related API wart: `cmux identify --json` reports **both** `tab_ref` and `surface_ref` for the same pane with the same number (e.g. `tab:30` / `surface:30`), and the CLI still carries `tab-action` — "tab" survives as a near-synonym for surface in parts of the API. The vendored skills are pristine (see the bullet above), so this gloss lives here rather than being patched into `cmux/SKILL.md`.
- **cmux documentation, versions, and which source wins.** The tool self-reports its own doc locations — run `cmux docs settings` rather than hardcoding these; captured 2026-07-27 as a starting point: web `https://cmux.com/docs/configuration#cmux-json`; raw skill `https://raw.githubusercontent.com/manaflow-ai/cmux/main/skills/cmux/SKILL.md`; settings schema `https://raw.githubusercontent.com/manaflow-ai/cmux/main/web/data/cmux.schema.json`. **THREE revisions are in play and none of them track each other:** (1) the installed app — `cmux --version` prints a build hash (e.g. `cmux 0.64.20 (100) [14e3400b9]`) that pins the real running revision; (2) the vendored skills in repo-2, whose SHA + vendor date live in `cmux-custom-skills/VENDOR.md` (`fc093e03…`, 2026-07-23); (3) those raw URLs, which resolve to **`main` — i.e. the LATEST, not your installed version.** **Rule: for flag-level facts (does `new-surface` take `--command`? does `send` accept `\n`?), `cmux <command> --help` on the installed binary OUTRANKS all three** — it is the only source guaranteed to describe the code that will actually execute. Use the docs for *intent* (what a setting means, why a thing exists) and the CLI for *contract*. Re-vendor the skills with `cmux-custom-skills/sync-cmux-skills.sh`; measured 2026-07-27, the vendored `cmux/SKILL.md` (84 lines) was **64 changed lines** behind upstream `main` (52 lines) — a condensing prose rewrite, **no command or flag differences**, and the Workspace/Surface definitions byte-identical (so the backwards-reading vocabulary noted above is current upstream, not a stale-vendor artifact). Re-vendoring is a deliberate decision, not routine hygiene — see BACKLOG **N58**.
