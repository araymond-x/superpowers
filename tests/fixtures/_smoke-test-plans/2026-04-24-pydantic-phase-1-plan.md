...
pattern_references:
  - name: "cli-validation-pattern"
    source_files: ["skills/subagent-driven-development/scripts/validate-plan.py"]
    reason: "CLI validation pattern (argparse, JSON stdout, exit codes 0/1/2/3)"
  - name: "hook-json-wrapping-pattern"
    source_files: ["skills/writing-plans/scripts/plan-validation-gate-hook.sh"]
    reason: "Hook JSON-wrapping pattern (jq -Rs . for stderr)"
  - name: "test-subprocess-pattern"
    source_files: ["tests/unit/test_sdd_partner_gate.py"]
    reason: "Test subprocess pattern (subprocess.run, returncode assertions)"
  - name: "test-helper-pattern"
    source_files: ["tests/unit/sdd_test_helpers.py"]
    reason: "Test helper pattern (workspace setup, fixture creation)"
modules:
  - id: 1
    title: "Core Models"
    task_ids: [1, 2, 3, 4, 5]
...
tasks:
  - id: 1
    ...
    module_id: 1
  - id: 2
    title: "Base Classes + Schema Versioning Tests"
    module_id: 1
    depends_on: [1]
  - id: 3
    title: "Plan Model + Tests"
    module_id: 1
    depends_on: [2]
    pattern_references: ["test-subprocess-pattern"]
  - id: 4
    title: "HandoffPackage Model + Tests"
    module_id: 1
    depends_on: [2]
  - id: 5
    title: "Error Formatter + Tests"
    module_id: 1
    depends_on: [1]
  - id: 6
    ...
    module_id: 2
    depends_on: [3, 5]
    pattern_references: ["cli-validation-pattern"]
  - id: 7
  ...
  - id: 8
    ...
    module_id: 2
    depends_on: [7]
    pattern_references: ["hook-json-wrapping-pattern"]
  - id: 9
