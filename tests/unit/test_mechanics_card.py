...
def _run_card(wt, feat, env_extra=None):
    import os
    ...
    if env_extra:
        env.update(
            env_extra
        )  # merged AFTER the ambient strip — deliberate injection only
...
def _materialize_minimal_plan(wt, feat):
    ...
    subprocess.run(
        [
            VENV_PY,
            str(SCRIPTS / "materialize-manifest.py"),
            "--plan-file",
            str(feat / "plan.md"),
            "--feature-dir",
            str(feat),
...
def test_card_deterministic_with_contents(tmp_path):
    ...
    assert (
        "controller-checkpoint.py" in card
        and "--phase pre-dispatch" in card
        and "--phase pre-completion" in card
        and "--manifest" in card
        and "--deviations-file" in card
        and "--reports-dir" in card
    )  # N35: both hard-required even in manifest mode
    assert "docs/imp-plans/feat/plan.md" in card and "deviations.md" in card
    assert (
        re.search(r"hops used:\s*1", card)
        and re.search(r"expected:\s*2", card)
...
def test_report_skeleton_passes_validate_report(tmp_path):
    ...
    r = subprocess.run(
        [VENV_PY, str(SCRIPTS / "validate-report.py"), "--report-file", str(skel)],
        capture_output=True,
        text=True,
    )
...
def test_card_regen_line_uses_real_interpreter(tmp_path):
    """N85: the standalone-regen line must show sys.executable's resolved
    path (matching the checkpoint invocation lines just above it in the same
    card), not the literal unexpanded '$PYTHON' string. VENV_PY is a symlink
    (worktree .venv -> main checkout .venv), so compare against the REAL
    path Python itself reports for that interpreter, not the invoked string."""
    real_interpreter = subprocess.run(
    ...
    assert _run_card(wt, feat).returncode == 0
    card = (reports / "handoff-mechanics.md").read_text()
    assert "$PYTHON " not in card
    assert real_interpreter in card


def test_card_ceiling_validates_max_hops(tmp_path):
    """N85: a non-numeric SUPERPOWERS_CMUX_MAX_HOPS must be REJECTED, same as
    spawn-handoff-session.sh's ^[0-9]+$ validation — the card must fall back
    to the derived default rather than echoing the garbage value verbatim."""
    wt, feat, reports = _fixture_feature(tmp_path)

    assert (
        _run_card(
            wt, feat, env_extra={"SUPERPOWERS_CMUX_MAX_HOPS": "banana"}
        ).returncode
        == 0
    )
    card = (reports / "handoff-mechanics.md").read_text()
    ...
    assert re.search(
        r"ceiling:\s*6", card
    )  # derived default for expected_hops=2 (unchanged)

    assert (
        _run_card(wt, feat, env_extra={"SUPERPOWERS_CMUX_MAX_HOPS": "9"}).returncode
        == 0
    )
    card = (reports / "handoff-mechanics.md").read_text()
    assert re.search(r"ceiling:\s*9", card)
