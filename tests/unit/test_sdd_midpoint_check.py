...
Legacy plan-globbing midpoint tests removed with Item 5 (legacy path removal);
manifest-mode midpoint is covered by TestContextSummaryBlocking.

The remaining test covers a bug found in the min-payment-extraction SDD session
(2026-04-10):

  Multi-line grep bug: `grep -c <pat> <file> || echo "0"` produces
...
class TestCheck6bMultiLineGrepBug:
    ...
    def test_tolerates_plan_file_with_zero_task_headers(self, tmp_path):
        ...
        runtime crash or any other bash syntax error. (Runs in manifest mode
        via the migrated setup_full_sdd_workspace; midpoint comes from the
        manifest's enforcement.context_summary_at, not plan globbing.)
        """
        tmpdir = str(tmp_path)
