...
**A third variant: `find` doesn't follow a symlinked top-level path argument** (2026-07-14). The entire skills install is one parent symlink (`~/.claude/skills/superpowers` → the fork's `skills/` directory). On macOS/BSD `find`, `find ~/.claude/skills/superpowers -name X` treats that symlink as a leaf node and does not descend into it — `find ~/.claude/skills/superpowers -type f` returns 0 results (not an error), while `find -L ~/.claude/skills/superpowers -type f` returns the real 139 files (verified live). Every other way of touching the path — `ls`, `cat`, `bash <path>`, the `Read` tool, or a plain absolute reference in skill text — dereferences the symlink transparently; `find` alone silently doesn't. An agent hit this executing brainstorming's Step 7.5 (`check-distillation.sh`), concluded the script "wasn't in `~/.claude/skills/superpowers`," and fell back to searching the fork by name — self-corrected, but burned a round-trip that `-L` would have avoided. No occurrence currently exists in this fork's own scripts/docs (checked 2026-07-14); if you ever write a `find` against any `~/.claude/skills/<name>` path, use `find -L`, not bare `find`.
...
| Scope fence dropped or demoted during spec distillation | 2026-07-07 telemetry-exp stable-fact-store: spec.md §1.2 out-of-scope list had no counterpart in spec-distilled.md (review finding M1); synthetic baseline demoted the fence to one line inside Contract Facts, heading + deferral targets lost — the distiller was following the rules *correctly* | Negative contract material (out-of-scope/non-goals/deferred/"do not modify" lists) needs its own explicit distillation category — otherwise it pattern-matches "rejected alternatives" stripping, and every layer leans the same wrong way (rule text, template, reviewer artifact-check, checker greps). Fix (N38): Distillation Rule 3 + `## Out of scope — do not build` template section + reviewer fence check with not-an-artifact carve-out + `check-distillation.sh <distilled> <full>` two-arg FAIL. When adding a "strip X" rule to any skill, name what X does NOT include. |
