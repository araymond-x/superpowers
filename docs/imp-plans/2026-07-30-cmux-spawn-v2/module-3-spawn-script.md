...
- [ ] **Step 1b: Close the TWO deferred residuals from Task 8's quality re-review round 3.** These are STEPS, not background reading — they are rows in `deviations.md` marked `Pending — TASK 9`, and a finding that no checkbox commands is a finding nobody does (this module already lost two `_handoff_support.py` paths to exactly that, which BLOCKED a partner round). Report on each by id.
...
- [ ] **Step 4: Run the FULL unit suite** — not "both unit files": Steps 1b–1d touch `test_spawn_handoff_v2.py`, `test_spawn_handoff.py` **and** `test_spawn_handoff_hardening.py`, and Step 3 rewrites the spawn core that other suites exercise. All PASS, old file fully migrated (baseline before this task: **748**; re-measure, do not inherit that number). **Step 5: Commit** — stage the FIVE explicit paths this task writes (never `git add -A`): `spawn-handoff-session.sh`, `spawn_handoff_helpers.py`, `test_spawn_handoff_v2.py`, `test_spawn_handoff.py`, `test_spawn_handoff_hardening.py` — enumerate against what you actually changed rather than copying this list — message `"feat(cmux-spawn-v2): surface topology + shared launch wrapper + workspace-create fallback"`.
