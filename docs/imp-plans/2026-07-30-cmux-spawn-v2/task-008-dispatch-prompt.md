...
**P7-4 — `stall-streak --tasks-done` is `type=int`, so it cannot accept the `unknown` that `tasks-done` legitimately prints.** The two CLI contracts do not compose. **Verify before acting:** the plan's step (e) already branches on `unknown` before calling `stall-streak`. Confirm that empirically. If the branch fully covers it, say so and close the row as already-satisfied; if not, either branch in the shell or accept the sentinel in `--tasks-done`.
...
**Additionally, for each of the ELEVEN rows above, state explicitly in your report: fixed / already-satisfied / not-done-and-why, with the evidence.** A row you silently drop is a row that is lost — `transition-module.py` archives reports at the module boundary.
