...
| **Minor 1** (misleading SSOT test) | **Fixed — made real, not renamed** | five: seed `6`→`9`; clamp `-lt 6 … DERIVED=6`→`9`; `* 2`→`* 99`; re-inserted the historical **indented, semicolon-joined** copy; and a **faithful two-copy reconstruction** whose else-branch derives into `MAX_HOPS` | **RED** ×5 on `test_shared_constants_are_the_ssot_the_shell_mirrors`, green at HEAD. Floor drift, factor drift, and re-duplication in either historical shape. **The first version of this assertion FAILED the last two** — see Self-Review. |
...
- **The SSOT test's first version caught only the duplication shape I invented, and missed the one the codebase actually produced — the same defect class as the Minor 1 it was fixing.** Its anchors were line-shaped (`^DERIVED=6$`, a trailing `$` after the arithmetic). My re-duplication control inserted `DERIVED=6` unindented at column 0 and went RED, so the claim "re-duplicating the derivation also fails" looked proven. It was not: the duplication that had actually existed was **indented and semicolon-joined**, and derived into `MAX_HOPS` in one copy and `DERIVED` in the other. Re-inserting either real shape **SURVIVED**. Rewritten to strip comment lines and then match the **construct** anywhere in executable text without keying on the target variable's name, asserting counts (`factor == 1`, `floor_cmp == 1`, `seeds == 2`) rather than line layouts. Both historical shapes now go RED. **A control you designed to match your fix is not a control** — it proves the assertion matches something, not that it matches what the codebase can produce.
- **The regexes were also nearly a false pass in the other direction.** The derivation's own SSOT comment quotes both literals ("max(6, 2 x expected)", "the literals 6 and 2"), the malformed-hop-counter comment contains `MAX_HOPS=0`, and the file carries `MAX_STALL_HOPS_DEFAULT=1`, `QUOTA_MIN_PCT_DEFAULT=15`, `QUOTA_TIMEOUT_DEFAULT=60`. Comment-stripping is load-bearing, so it has its own positive control inside the test (`assert "EXPECTED_HOPS" in code`) — a stripper that removed too much would otherwise make every count assertion pass vacuously.
...
3. **`BUDGET_FLAG` remains the one new SC2034 — re-measured, not carried forward on trust.** `shellcheck --severity=warning --external-sources` on the restructured file returns **exactly one** finding and it is still `BUDGET_FLAG` (the derivation collapse and the new `_total_raw` block added none). Task 9 must consume it or remove it.
...
6. **`validate-report.py` reports `has_deviations: false` / `has_concerns: false` on this report — and on the ORIGINAL Task 8 report too** (verified by running it against both). Not a regression and not an empty section: `section_contains_content`'s bold-header regex terminates its body capture at the next `**[A-Z]`, and both reports open those sections with bold text, so the captured body is ≤10 chars and is classified empty. Flagged because a controller reading the JSON rather than the prose would conclude there are no deviations to record, when there are four and six.
7. Status is `DONE_WITH_CONCERNS`, matching the original: Minor 5 is deliberately not done (Module 4's scope), Minor 3 is a deliberate non-fix, and `BUDGET_FLAG` is still open on Task 9.
8. Two commits: `2f677e6` (fixes) and a follow-up for the SSOT-test shape-tolerance correction.
