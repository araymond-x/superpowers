...
def transition(manifest_path: str, completed_module: str, next_module: str) -> int:
    ...
    # Determine git root from manifest path
    git_root = str(manifest_file.parent.parent.parent)  # feature_dir/../..
...
class TestManifestMode:
    ...
    def test_manifest_overrides_plan_file(self, tmp_path):
        ...
        # Create manifest with specific plan_file path
        # Run checkpoint with --manifest
        # Verify it reads the plan from the manifest path, not --plan-file
        pass  # Implement with actual workspace setup

    def test_micro_tier_skips_honesty_check(self, tmp_path):
        """Pre-completion with micro tier should SKIP honesty check."""
        # Create micro-tier manifest
        # Run checkpoint --phase pre-completion --manifest
        # Verify honesty_check status is SKIP
        pass  # Implement with actual workspace setup

    def test_backward_compat_without_manifest(self, tmp_path):
        """When --manifest absent, --plan-file works as before."""
        # Run checkpoint with --plan-file only (no --manifest)
        # Verify existing behavior unchanged
        pass  # Implement with actual workspace setup
```

Note: The actual test implementations will follow the patterns in `test_controller_checkpoint_stale.py` — creating plan files, deviations, reports in a temp directory and running the checkpoint script via subprocess.
