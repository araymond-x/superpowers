...
- [x] `spawn-handoff-session.sh <bundle-id> --dry-run` in a real picker-launched cmux SDD session prints a composed successor command with the same version, decoded+re-quoted forwarded args, and correctly incremented label — and spawns/increments nothing. **(Verified live 2026-07-28. Two sub-clauses were vacuous in that first run; a SECOND live smoke the same day closed one of them. (a) `decoded+re-quoted forwarded args` is now covered by a REAL run — a session launched with `--append-system-prompt-file` produced a non-empty `CLAUDE_CODE_PICKER_ARGS`, and the forwarded flag was proven to reach the successor's model context by a canary phrase that existed in exactly one file on disk and came back out of the successor's mouth unprompted. (b) `correctly incremented label` is STILL VACUOUS — both live runs had a label with no `-Session-N` suffix, so the seed branch fired, not the increment branch; note the seed output `…-Session-2` is string-identical to an increment from `…-Session-1`, so the log line alone cannot tell them apart. (b) remains covered only by a labelled synthetic dry-run probe. See the `Task 11 (feature close)` and `Task 11 (post-merge smoke #2)` deviations rows.)**
