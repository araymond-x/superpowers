...
- **Brainstorming path** — `brainstorming/SKILL.md` step 3.5, immediately after the feature name is established: present the mode choice (default `auto`, options `ask` / `off`-manual), record the answer for the writing-plans handoff. Follows the same "press enter to accept the default" interaction as the feature-name prompt.
