...
3. **N5 — Make `TASK_HEADER_PATTERN` fence-aware (strengthen).** `^###\s+Task\s+(\d+)` (validate-plan.py L58; reused for task counting in controller-checkpoint.py) matches `### Task 91/93/94/95` *inside fenced code fixtures* in a plan, inflating both the verification-ratio denominator (Check 8) and the pre-completion checkbox count. Skip headers inside ``` fenced blocks. Low risk, removes a silent miscount.
