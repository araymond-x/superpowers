...
1. **N3 — Check 4c (dispatch provenance) rejects valid plans whose first in-scope task isn't Task 0 (strengthen).** This module-boundary incompatibility surfaced here is **one of two manifestations of a single root cause**: Check 4c's exemption guard `TASK_NUMBER -gt 0` assumes the first in-scope task is always Task 0. (a) *Module boundaries* — `transition-module.py` Step 5 truncates the live dispatch log, so the next module's first task BLOCKs on the now-archived N-1 reviews (this execution abandoned the prescribed script, deviations.md Module 1→2, 2→3). (b) *No-Task-0 plans* — a contract-free plan validly starts at Task 1, and Check 4c then greps the log for a `task=0` review that can never exist (surfaced live in the practerus marketing-cta-guard run, 2026-05-31). **Canonical fix (covers both):** give Check 4c a parallel exemption — skip the PREV provenance check when `PREV < MANIFEST_TASK_START` (no prior in-scope task), preserving full enforcement for plans that *do* declare Task 0 and for non-first tasks within a module. (Alternative module-only patches — preserve provenance across the boundary, or have Check 4c read `archive-<module>/.dispatch-log` — are narrower and don't fix the no-Task-0 case.) Reconciled with the practerus finding under one BACKLOG N3; the single highest-signal item.
