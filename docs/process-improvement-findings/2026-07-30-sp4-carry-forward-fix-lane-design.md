...
### Three flat lookup sites a re-opened task would trip (a-file)
...
(`IMPL_GLOB`/`SPEC_GLOB`/`QUAL_GLOB` via `task_report_glob`), which fire first** — and require an
explicit ledger row naming the finding.

**Check 4's globs are the one a designer will miss.** Their skip keys on
`TASK_NUMBER -eq MANIFEST_TASK_START`, whereas Check 4c keys on `PREV -lt MANIFEST_TASK_START`.
Admitting a below-range id is exactly what pulls the two apart — Check 4 blocks on archived files
while Check 4c waves the same task through.
